...
const Email = ({ email, setStarredEmail, selectedEmails, setSelectedEmails }) => {
    ...
    const toggleStarredEmail = async () => {
        try {
            const response = await toggleStarredEmailService.call({ id: email._id, value: !email.starred });
            const updatedEmail = response.data; 
            setStarredEmail(updatedEmail);
        } catch (error) {
            console.error('Error toggling starred email:', error.message);
        }
    };
